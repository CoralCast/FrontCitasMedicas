...
export default function Recepcion() {
  ...
  const citasFiltradas = useMemo(() => {
    ...
    if (!search) return citasOrdenadas
  ...
  const pacientesActivos = pacientes.filter((paciente) => paciente.activo !== false)
  ...
  const gruposPacientes = useMemo(() => {
    ...
    return [...groups.entries()]
      ...
      .sort((a, b) => a.titular.nombre_completo.localeCompare(b.titular.nombre_completo))
  }, [pacientesActivos])
  ...
  useEffect(() => {
    async function loadRescheduleAvailability() {
      ...
      try {
        const data = await getDisponibilidad(selectedCita.doctor.id_doctor, rescheduleForm.fecha)
  ...
  function updateForm(field, value) {
    setForm((current) => ({
      ...current,
      [field]: value,
      ...(field === "id_doctor" || field === "fecha" ? { hora_inicio: "" } : {}),
    }))
  }
  ...
  async function handleReschedule(event) {
    ...
    if (!selectedCita || !rescheduleForm.hora_inicio) {
      setError("Selecciona un horario disponible para reagendar.")
      return
    }
    ...
    <div className="reception-page min-h-screen bg-gray-100 text-slate-900">
            ...
            <nav className="space-y-3">
              <button
                ...
                className={`reception-nav-item flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-bold ${
                  activeView === "agenda" ? "bg-blue-50 text-blue-700" : "text-slate-500 hover:bg-slate-50"
                }`}
              >
              ...
              <button
                ...
                className={`reception-nav-item flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-bold ${
                  activeView === "nueva" ? "bg-blue-50 text-blue-700" : "text-slate-500 hover:bg-slate-50"
                }`}
              >
              ...
              <button
                ...
                className={`reception-nav-item flex w-full items-center gap-3 rounded-xl px-4 py-3 text-left text-sm font-bold ${
                  activeView === "pacientes" ? "bg-blue-50 text-blue-700" : "text-slate-500 hover:bg-slate-50"
                }`}
              >
                <span className="flex h-6 w-6 items-center justify-center rounded-md bg-slate-100 text-xs">
                  3
        ...
        <main className="reception-main flex-1 bg-gray-50">
          <header className="reception-header border-b border-slate-100 bg-white px-8 py-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <h2 className="text-3xl font-semibold">{viewTitle[activeView]}</h2>
          ...
          <section className="px-8 py-8">
              ...
              <div className="space-y-6">
                <div className="grid gap-4 md:grid-cols-3">
                  <article className="reception-card rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                    ...
                    <p className="mt-4 text-4xl font-semibold">{citasHoy.length}</p>
                  </article>
                  <article className="reception-card rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                    <p className="text-sm font-bold text-slate-500">Pendientes</p>
                    <p className="mt-4 text-4xl font-semibold text-blue-700">{citasPendientes.length}</p>
                  </article>
                  <article className="reception-card rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
                    <p className="text-sm font-bold text-slate-500">Pacientes activos</p>
                    <p className="mt-4 text-4xl font-semibold">{pacientesActivos.length}</p>
                  </article>
                </div>
                ...
                <section className="reception-card rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                  ...
                  {loading ? (
                          ...
                          {citasFiltradas.map((cita) => (
                            <tr key={cita.id_cita} className="bg-white">
                              <td className="px-4 py-4">
                                <p className="font-semibold">{formatDate(cita.fecha)}</p>
                                <p className="text-xs font-semibold text-slate-400">
                                  {timeText(cita.hora_inicio)} - {timeText(cita.hora_fin)}
                                </p>
                              </td>
                              <td className="px-4 py-4">
                                <p className="font-semibold">{cita.paciente?.nombre_completo}</p>
                                <p className="text-xs text-slate-500">{cita.motivo}</p>
                              </td>
                              <td className="px-4 py-4">
                                <p className="font-bold">{cita.doctor?.nombre_completo}</p>
                                <p className="text-xs text-slate-500">{cita.doctor?.especialidad}</p>
                              </td>
                              <td className="px-4 py-4">
                                <span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${statusClass(cita.estado)}`}>
                                  {cita.estado}
                                </span>
                              </td>
                              <td className="px-4 py-4">
                    ...
                    <div className="rounded-xl border-2 border-dashed border-slate-200 bg-white p-8 text-center">
                      <p className="font-semibold">No hay citas para mostrar.</p>
            ...
            {activeView === "nueva" && (
              <form onSubmit={handleCreateCita} className="reception-card mx-auto max-w-3xl rounded-2xl border border-slate-100 bg-white p-8 shadow-sm">
                <div className="mb-7">
                  <h3 className="text-2xl font-semibold">Registrar cita</h3>
                  <p className="text-sm font-semibold text-slate-500">
                    Selecciona paciente, doctor, fecha disponible y motivo de consulta.
                  </p>
                </div>

                <div className="grid gap-5 md:grid-cols-2">
                  <div className="md:col-span-2">
                    <label className="mb-2 block text-sm font-bold text-slate-600">Paciente</label>
                    <select
                      value={form.id_paciente}
                      onChange={(event) => updateForm("id_paciente", event.target.value)}
                  ...
                  {selectedPatientGroup && selectedPatientGroup.integrantes.length > 1 && (
                      ...
                      <div className="mt-3 grid gap-2 md:grid-cols-2">
                        {selectedPatientGroup.integrantes.map((paciente) => (
                          <button
                            ...
                            onClick={() => updateForm("id_paciente", paciente.id_paciente)}
                          ...
                          >
                            ...
                            <span className="uppercase">{paciente.parentesco}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                  ...
                  <div>
                    <label className="mb-2 block text-sm font-bold text-slate-600">Doctor</label>
                    <select
                      value={form.id_doctor}
                      onChange={(event) => updateForm("id_doctor", event.target.value)}
                      className="w-full rounded-xl bg-slate-100 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500"
                      required
                    >
                  ...
                  <div>
                    <label className="mb-2 block text-sm font-bold text-slate-600">Fecha</label>
                    <input
                      type="date"
                      min={today}
                      value={form.fecha}
                  ...
                  <div className="md:col-span-2">
                    <label className="mb-2 block text-sm font-bold text-slate-600">Horario</label>
                    <select
                      ...
                      required
                    >
                      <option value="">{loadingHorarios ? "Consultando..." : "Seleccionar horario"}</option>
                      {horarios.map((horario) => (
                        <option key={`${horario.hora_inicio}-${horario.hora_fin}`} value={horario.hora_inicio}>
                  ...
                  <div className="md:col-span-2">
                    <label className="mb-2 block text-sm font-bold text-slate-600">Motivo</label>
                    <textarea
                      value={form.motivo}
              ...
              <section className="reception-card mx-auto max-w-4xl rounded-2xl border border-slate-100 bg-white p-8 shadow-sm">
                ...
                <div className="space-y-4">
                  {gruposPacientes.map((grupo) => (
                    <article key={grupo.idUsuario} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                      <button
                        type="button"
                        onClick={() =>
                      ...
                      >
                        <div>
                          <p className="text-lg font-semibold">{grupo.titular.nombre_completo}</p>
                          <p className="text-sm font-semibold text-slate-500">
                            Paciente titular - {grupo.titular.curp}
                          </p>
                        </div>
                        <span className="rounded-full bg-white px-4 py-2 text-xs font-semibold text-blue-700">
                          {grupo.integrantes.length} paciente{grupo.integrantes.length === 1 ? "" : "s"}
                        </span>
                      </button>
                        ...
                        <div className="mt-4 grid gap-3 border-t border-slate-200 pt-4 md:grid-cols-2">
                          {grupo.integrantes.map((paciente) => (
                            <button
                              ...
                              className="flex items-center justify-between rounded-lg bg-white px-4 py-3 text-left text-sm hover:bg-blue-50"
                            >
                              <div>
                                <p className="font-semibold text-slate-800">{paciente.nombre_completo}</p>
                                <p className="text-xs font-semibold text-slate-500">{paciente.curp}</p>
                              </div>
      ...
      {selectedCita && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-6">
          <form onSubmit={handleReschedule} className="w-full max-w-xl rounded-2xl border border-slate-100 bg-white p-6 shadow-2xl">
            <h2 className="text-2xl font-semibold">Reagendar cita</h2>
            <p className="mt-1 text-sm font-semibold text-slate-500">
              {selectedCita.paciente?.nombre_completo} con {selectedCita.doctor?.nombre_completo}
            </p>

            <label className="mt-6 mb-2 block text-sm font-bold text-slate-600">Nueva fecha</label>
            <input
              type="date"
              min={today}
              value={rescheduleForm.fecha}
              onChange={(event) =>
                setRescheduleForm({ fecha: event.target.value, hora_inicio: "" })
              }
              className="w-full rounded-xl bg-slate-100 px-4 py-3 outline-none focus:ring-2 focus:ring-blue-500"
              required
            />

            <label className="mt-5 mb-2 block text-sm font-bold text-slate-600">Nuevo horario</label>
            <select
            ...
            >
              <option value="">
                {loadingHorariosReagenda ? "Consultando..." : "Seleccionar horario"}
              </option>
